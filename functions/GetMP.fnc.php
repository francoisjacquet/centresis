...
{	global $_CENTRE;
	...
	if(!$_CENTRE['GetCurrentMP'][$date][$mp])
	 	$_CENTRE['GetCurrentMP'][$date][$mp] = DBGet(DBQuery("SELECT MARKING_PERIOD_ID FROM SCHOOL_MARKING_PERIODS WHERE MP='$mp' AND '".date("Y-m-d", strtotime($date))."' BETWEEN START_DATE AND END_DATE AND SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'"));
