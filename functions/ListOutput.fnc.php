...
function ListOutput($result,$column_names,$singular='.',$plural='.',$link=false,$group=array(),$options=array())
{
	...
	if($_LIST['output']!=false)
	{
		...
		if($result_count!=0 || ($_REQUEST['LO_search'] && $_REQUEST['LO_search']!=_('Search')))
		{
			...
			// SHADOW
			if(!isset($_REQUEST['_CENTRE_PDF']))
				echo '<TABLE width=100% cellpadding=0 cellspacing=0><TR><TD width=4 height=100% rowspan=2>&nbsp;</TD><TD></TD><TD width=4 height=100% rowspan=2></TD></TR><TR><TD>';
			...
			{
				echo "<TD class='cont_submenu' bgcolor=$options[header_color]><DIV id=LOx$i style='position: relative;'></DIV></TD>";
				$i++;
			}
			...
			if($result_count!=0 && $cols && !isset($_REQUEST['_CENTRE_PDF']))
			{
				foreach($column_names as $key=>$value)
				{
					if($_REQUEST['LO_sort']==$key)
						...
						$direction = 1;
					echo "<TD class='cont_submenu' bgcolor=".($options['header_colors'][$key]?$options['header_colors'][$key]:$options['header_color'])."><DIV id=LOx$i style='position: relative;'></DIV>";
					echo "<A ";
					if($options['sort'])
			...
			if($result_count!=0 && $link['add']['first'] && ($stop-$start+1)>=$link['add']['first'])
			{
				...
				elseif($link['add']['html'] && $cols)
				{
					...
					if($remove && !isset($_REQUEST['_CENTRE_PDF']) && $link['add']['html']['remove'])
						echo "<TD class='cont_submenu' bgcolor=$color>".$link['add']['html']['remove']."</TD>";
					elseif($remove && !isset($_REQUEST['_CENTRE_PDF']))
						echo "<TD class='cont_submenu' bgcolor=$color>".button('add')."</TD>";

					foreach($column_names as $key=>$value)
					{
						echo "<TD class='cont_submenu' bgcolor=$color class=LO_field>".$link['add']['html'][$key]."</TD>";
					}
					echo "</TR>";
					$count++;
				}
					...
					foreach($column_names as $key=>$value)
					{
						if($link[$key] && $item[$key]!==false && !isset($_REQUEST['_CENTRE_PDF']))
						{
							...
							if($color==Preferences('HIGHLIGHT'))
								...
								echo ($color=='#464646') ? '<font color=#FFFFFF>' : '<font color=#0000FF>';
							echo $item[$key];
							if(!$item[$key])
								echo '***';
							echo '</font>';
						...
						else
						{
							...
							if($color==Preferences('HIGHLIGHT'))
								...
								echo ($color=='#464646') ? '<style>.contrast .LO_field { color:#FFFFFF !important; }</style><font color=#FFFFFF class=contrast>' : '<font color=#000000>';
							echo $item[$key];
							if(!$item[$key])
								echo '&nbsp;';
							#if($color==Preferences('HIGHLIGHT'))
		...
		if($result_count==0)
		{
			...
			elseif(($link['add']['html'] || $link['add']['span']) && count($column_names) && !isset($_REQUEST['_CENTRE_PDF']))
			{
				...
				echo '<TABLE width=100% cellpadding=0 cellspacing=0><TR></TR><TR><TD>';
				if($link['add']['html'])
				{
					echo "<TABLE cellpadding=$options[cellpadding] width=100%><TR><TD bgcolor=$options[header_color]></TD>";
					foreach($column_names as $key=>$value)
					{
						echo "<TD class='cont_submenu' bgcolor=".($options['header_colors'][$key]?$options['header_colors'][$key]:$options['header_color'])."><A class=column_heading><b>" . str_replace(' ','&nbsp;',$value) . "</b></A></TD>";
					}
					echo "</TR>";
		...
		if($result_count!=0)
		{
			...
			if($remove && !isset($_REQUEST['_CENTRE_PDF']) && $result_count!=0)
			{
				echo "<TD class='cont_submenu' bgcolor=$options[header_color] id=LO_col$i></TD>";
				$i++;
			}
			...
			if($cols && !isset($_REQUEST['_CENTRE_PDF']))
			{
				foreach($column_names as $key=>$value)
				{
					echo "<TD class='cont_submenu' bgcolor=".($options['header_colors'][$key]?$options['header_colors'][$key]:$options['header_color'])." id=LO_col$i><A class=column_heading><b>".str_replace('controller','',$value).'</b></A></TD>';
					$i++;
				}
			}
			echo '</TR></TABLE>';
