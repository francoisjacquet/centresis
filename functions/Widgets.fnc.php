		...
		switch($item)
		{
			...
			case 'eligibility':
				...
				if($_REQUEST['ineligible']=='Y')
				{
					...
					$start_date = strtoupper(date('Y-m-d',time() - ($today-$START_DAY)*60*60*24));
					$end_date = strtoupper(date('Y-m-d',time()));
					$extra['WHERE'] .= " AND (SELECT count(*) FROM ELIGIBILITY e WHERE ssm.STUDENT_ID=e.STUDENT_ID AND e.SYEAR=ssm.SYEAR AND e.SCHOOL_DATE BETWEEN '".date("Y-m-d",strtotime($start_date))."' AND '".date("Y-m-d",strtotime($end_date))."' AND e.ELIGIBILITY_CODE='FAILING') > '0'";
					if(!$extra['NoSearchTerms'])
