...
function DrawTab($title,$link='',$tabcolor='#DDDDDD',$textcolor='#FFFFFF',$type='',$rollover='')
{
	...
	$growee_class = ($tabcolor=="#333366")?'growee':'';
	$block_table .= "<table border=0 cellspacing=0 cellpadding=0>";
	$block_table .= "  <tr class='mini-window' style='line-height:0;border:0;' id=tab[".ereg_replace('[^a-zA-Z0-9]','_',$link)."]>";
	$block_table .= "    <td style=\"background-color:$tabcolor;\" class=\"BoxHeading $growee_class\" valign=middle>";
	if($link)
	{
		...
		if(!isset($_REQUEST['_CENTRE_PDF']))
			$block_table .= "<A HREF='$link' class=BoxHeading style='color:$textcolor !important;' $rollover id=tab_link[".ereg_replace('[^a-zA-Z0-9]','_',$link)."]>$title</A>";
		else
