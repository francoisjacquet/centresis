...
function DateInput($value,$name,$title='',$div=true,$allow_na=true)
{
	...
	if(AllowEdit() && !$_REQUEST['_CENTRE_PDF'])
	{
		if($value=='' || $div==false)
			...
			return "<DIV id='div$name'><div onclick='javascript:addHTML(\"".str_replace('"','\"',PrepareDate($value,"_$name",$allow_na,array('Y'=>1,'M'=>1,'D'=>1))).($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').str_replace(array("'",'"'),array('&#39;','\"'),$title).(strpos(strtolower($title),'<font ')===false?'</FONT>':'').'</small>':'')."\",\"div$name\",true)'><span style='border-bottom-style:dotted;border-bottom-width:1px;border-bottom-color:".Preferences('TITLES').";'>".($value!=''?ProperDate($value):'-').'</span>'.($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').$title.(strpos(strtolower($title),'<font ')===false?'</FONT>':'').'</small>':'').'</div></DIV>';
	}
	else
...
function UploadInput($value,$name,$title='',$options='',$div=true) #Added 07/18/2013 for Admin Settings upload purposes
{
	$ret = '';
	$ret .= "<script type='text/javascript'>$(function() { $ = jQuery.noConflict(); $('#".$name."').uploadify({ 'multi':false,'auto':false, buttonImage: 'modules/Admin/includes/uploadify/browse-btn.png', 'cancelImage': 'modules/Admin/includes/uploadify/cancel.png', 'swf' : 'modules/Admin/includes/uploadify/uploadify.swf', 'uploader' : 'modules/Admin/includes/uploadify/uploadify-nm.php', 'scriptData' : { path : '../uploads' } }); });</script>";

		$ret .= "<input type='file' name='$name' id='$name' />";
		$ret .= "<a href=\"javascript:$('#$name').uploadify('upload')\">Upload Logo</a>";
	return $ret;
}

function IMGInput($value,$name,$title='',$options='',$div=true) #Added 07/18/2013 for Admin Settings upload purposes
{
	$ret = '';
	$ret .= "<img border='0' style='max-width:320px;' name='$name' src='".$value."' ".$options.">".($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').$title.(strpos(strtolower($title),'<font ')===false?'</FONT>':'').'</small>':'');
	return $ret;
}
...
function TextInput($value,$name,$title='',$options='',$div=true)
{
	...
	if(AllowEdit() && !$_REQUEST['_CENTRE_PDF'])
	{
		...
		if(trim($value)=='' || $div==false)
			...
			$value = DecryptPWD($value);
			return "<DIV id='div$name'><div onclick='javascript:addHTML(\"<INPUT type=text id=input$name name=$name ".($value||$value==='0'?'value=\"'.str_replace(array("'",'"'),array('&#39;','&rdquo;'),$value).'\"':'').' '.$options.'>'.($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').str_replace(array("'",'"'),array('&#39;','\"'),$title).(strpos(strtolower($title),'<font ')===false?'</FONT>':'').'</small>':'')."\",\"div$name\",true); document.getElementById(\"input$name\").focus();'><span style='border-bottom-style:dotted;border-bottom-width:1px;border-bottom-color:".Preferences('TITLES').";'>".($value!=''?$value1:'-').'</span>'.($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').$title.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').'</small>':'').'</div></DIV>';

			elseif($name=='students[PASSWORD]' || $name=='values[PEOPLE][PASSWORD]') :
			$value = DecryptPWD($value);
			return "<DIV id='div$name'><div onclick='javascript:addHTML(\"<INPUT type=text id=input$name name=$name ".($value||$value==='0'?'value=\"'.str_replace(array("'",'"'),array('&#39;','&rdquo;'),$value).'\"':'').' '.$options.'>'.($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').str_replace(array("'",'"'),array('&#39;','\"'),$title).(strpos(strtolower($title),'<font ')===false?'</FONT>':'').'</small>':'')."\",\"div$name\",true); document.getElementById(\"input$name\").focus();'><span style='border-bottom-style:dotted;border-bottom-width:1px;border-bottom-color:".Preferences('TITLES').";'>".($value!=''?$value1:'-').'</span>'.($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').$title.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').'</small>':'').'</div></DIV>';
...
function TextAreaInput($value,$name,$title='',$options='',$div=true)
{
	...
	if(AllowEdit() && !$_REQUEST['_CENTRE_PDF'])
	{
		...
		if($value=='' || $div==false)
			...
			return "<DIV id='div$name'><div onclick='javascript:addHTML(\"<TEXTAREA id=textarea$name name=$name $options>".ereg_replace("[\n\r]",'\u000D\u000A',str_replace("\r\n",'\u000D\u000A',str_replace(array("'",'"'),array('&#39;','\"'),$value)))."</TEXTAREA>".($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').str_replace(array("'",'"'),array('&#39;','\"'),$title).(strpos(strtolower($title),'<font ')===false?'</FONT>':'').'</small>':'')."\",\"div$name\",true); document.getElementById(\"textarea$name\").value=unescape(document.getElementById(\"textarea$name\").value);'><TABLE class=LO_field height=100%><TR><TD>".((substr_count($value,"\r\n")>$rows)?'<DIV style="overflow:auto; height:'.(15*$rows).'px; width:'.($cols*9).'; padding-right: 16px;line-height:16px !important; border-bottom-style:dotted;border-bottom-width:1px;border-bottom-color:'.Preferences('TITLES').';">'.nl2br($value).'</DIV>':'<DIV style="overflow:auto; width:'.($cols*9).'; padding-right: 16px;line-height:16px !important; border-bottom-style:dotted;border-bottom-width:1px;border-bottom-color:'.Preferences('TITLES').';">'.nl2br($value).'</DIV>').'</TD></TR></TABLE>'.($title!=''?'<small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').$title.(strpos(strtolower($title),'<font ')===false?'</FONT>':'').'</small>':'').'</div></DIV>';
	}
	else
...
function CheckboxInput($value,$name,$title='',$checked='',$new=false,$yes='Yes',$no='No',$div=true,$extra='')
{
	...
	if(AllowEdit() && !$_REQUEST['_CENTRE_PDF'])
	{
		if($new || $div==false)
			...
			return "<DIV id='div$name'><div onclick='javascript:addHTML(\"<INPUT type=hidden name=$name value=\\\"\\\"><INPUT type=checkbox name=$name ".($value?'checked':'')." value=Y ".str_replace('"','\"',$extra).">".($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').str_replace(array("'",'"'),array('&#39;','\"'),$title).(strpos(strtolower($title),'<font ')===false?'</FONT>':'').'</small>':'')."\",\"div$name\",true)'><span style='border-bottom-style:dotted;border-bottom-width:1px;border-bottom-color:".Preferences('TITLES').";'>".($value?$yes:$no).'</span>'.($title!=''?'<BR><small>'.(strpos(strtolower($title),'<font ')===false?'<FONT color='.Preferences('TITLES').'>':'').$title.(strpos(strtolower($title),'<font ')===false?'</FONT>':'').'</small>':'')."</div></DIV>";
	}
	else
