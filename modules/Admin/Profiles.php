...
{
	//$id = DBGet(DBQuery("SELECT ".db_nextval('USER_PROFILES')." AS ID".FROM_DUAL));
	$id = db_nextval('USER_PROFILES');
...
if($_REQUEST['modfunc']!='delete')
{
	...
	echo '<TR><TD colspan=2 style="border-bottom:1px dotted #767676;"><small><b>'._('Profiles').'</b></small></TD></TR>';
	...
	if($_REQUEST['profile_id']!='')
	{
		...
		foreach($menu as $modcat=>$profiles)
		{
			...
			if($xprofile=='admin' || $modcat=='Students' || $modcat=='Scheduling' || $modcat=='Resources')
			...
			if(count($values))
			{
				foreach($values as $file=>$title)
				{
					if(!is_numeric($file))
					{
						...
						if($xprofile=='admin' || $modcat=='Resources' || $modcat=='Scheduling')
