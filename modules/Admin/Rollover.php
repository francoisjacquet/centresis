...
{	global $next_syear,$CentreModules;
	...
	switch($table)
	{
		...
		case 'SCHOOL_MARKING_PERIODS':
			...
			DBQuery("INSERT INTO SCHOOL_MARKING_PERIODS (PARENT_ID,SYEAR,MP,SCHOOL_ID,TITLE,SHORT_NAME,SORT_ORDER,START_DATE,END_DATE,POST_START_DATE,POST_END_DATE,DOES_GRADES,DOES_EXAM,DOES_COMMENTS,ROLLOVER_ID) SELECT PARENT_ID,SYEAR+1,MP,SCHOOL_ID,TITLE,SHORT_NAME,SORT_ORDER,DATE_ADD(START_DATE, INTERVAL 1 YEAR),DATE_ADD(END_DATE, INTERVAL 1 YEAR),DATE_ADD(POST_START_DATE, INTERVAL 1 YEAR),DATE_ADD(POST_END_DATE, INTERVAL 1 YEAR),DOES_GRADES,DOES_EXAM,DOES_COMMENTS,MARKING_PERIOD_ID FROM SCHOOL_MARKING_PERIODS WHERE SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'");
		...
		case 'ATTENDANCE_CALENDARS':
			...
			DBQuery("DELETE FROM attendance_calendar WHERE SCHOOL_ID='".UserSchool()."' AND SYEAR='$next_syear'");
			$fy_RET = DBGet(DBQuery("SELECT START_DATE,END_DATE FROM SCHOOL_MARKING_PERIODS WHERE MP='FY' AND SCHOOL_ID='".UserSchool()."' AND SYEAR='".$next_syear."'"));
			$fy_RET = $fy_RET[1];
				# Start Date
				$day_min = substr($fy_RET['START_DATE'],8,2);
				$month_min = substr($fy_RET['START_DATE'],5,2);
				$year_min = substr($fy_RET['START_DATE'],0,4);
				# End Date
				$day_max = substr($fy_RET['END_DATE'],8,2);
				$month_max = substr($fy_RET['END_DATE'],5,2);
				$year_max = substr($fy_RET['END_DATE'],0,4);

				$begin = mktime(0,0,0,MonthNWSwitch($month_min,'to_num'),$day_min*1,$year_min) + 43200;
				$end = mktime(0,0,0,MonthNWSwitch($month_max,'to_num'),$day_max*1,$year_max) + 43200;

				$weekday = date('w',$begin);
				$mon_to_fri = array(1,2,3,4,5);
				for($i=$begin;$i<=$end;$i+=86400)
				{
					if(in_array($weekday, $mon_to_fri))
						DBQuery("INSERT INTO ATTENDANCE_CALENDAR (SYEAR,SCHOOL_ID,SCHOOL_DATE,MINUTES,CALENDAR_ID) values ('".$next_syear."','".UserSchool()."','".date('Y-m-d',$i)."','999','".$CAL_RET[1]['LATESTID']."')");
					$weekday++;
					if($weekday==7)
						$weekday = 0;
				}
