...
{	global $THIS_RET;
	...
	$periods_RET = DBGet(DBQuery("SELECT COURSE_PERIOD_ID,MARKING_PERIOD_ID,CALENDAR_ID,TOTAL_SEATS FROM COURSE_PERIODS WHERE COURSE_ID='$THIS_RET[COURSE_ID]' AND (GENDER_RESTRICTION='N' OR GENDER_RESTRICTION='".substr($THIS_RET['CUSTOM_200000000'],0,1)."')".($THIS_RET['WITH_TEACHER_ID']?" AND TEACHER_ID='$THIS_RET[WITH_TEACHER_ID]'":'').($THIS_RET['NOT_TEACHER_ID']?" AND TEACHER_ID!='$THIS_RET[NOT_TEACHER_ID]'":'').($THIS_RET['WITH_PERIOD_ID']?" AND PERIOD_ID='$THIS_RET[WITH_PERIOD_ID]'":'').($THIS_RET['NOT_PERIOD_ID']?" AND PERIOD_ID!='$THIS_RET[NOT_PERIOD_ID]'":'')));
