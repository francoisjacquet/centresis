...
if($_REQUEST['subject_id'])
{
	...
	if($_REQUEST['course_id'])
	{
		...
		$header2 .= "&students=$location&modfunc=students&include_child_mps=$_REQUEST[include_child_mps]>List Students</A> || ".$header2."&unscheduled=true&students=$location&modfunc=students&include_child_mps=$_REQUEST[include_child_mps]>List Unscheduled Students</A>";
...
if($_REQUEST['modfunc']=='course_periods' || $_REQUEST['students']=='course_periods')
{
	...
	foreach($RET as $key=>$period)
	{
		...
		if($_REQUEST['include_child_mps'])
		{
			...
			{
				$value += array('OFT_'.$mp=>($total!==false?($filled_seats[$mp]!==false?$total-$filled_seats[$mp]:''):'n/a').'|'.($filled_seats[$mp]!==false?$filled_seats[$mp]:'').'|'.($total!==false?$total:'n/a'));
			}
		}
		else
    ...
    $columns = array('TITLE'=>_('Course'),'COUNT_REQUESTS'=>_('Requests'));
	$columns = array('TITLE'=>_('Period').' - '._('Teacher'));
	if($_REQUEST['include_child_mps'])
	{
		foreach(explode(',',GetAllMP()) as $mp)
		{
