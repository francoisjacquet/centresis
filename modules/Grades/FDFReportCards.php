...
foreach($mps_RET as $sem=>$quarters)
{
	...
	$extra['search'] .= '<TD><INPUT type=checkbox name=mp_arr[] value=E'.$sem.'>'.sprintf(_('%s Exam'),GetMP($sem,'SHORT_NAME')).'</TD><TD><INPUT type=checkbox name=mp_arr[] value='.$sem.' CHECKED>'.GetMP($sem,'SHORT_NAME').'</TD>';
	$extra['search'] .= '</TR>';
}
