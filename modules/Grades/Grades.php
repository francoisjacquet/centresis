...
else
{
	...
	if($_REQUEST['assignment_id']!='all')
		$LO_columns += array('STUDENT_ID'=>_('Centre ID'));
...
$type_select = '<SELECT name=type_id onchange="document.location.href=\'Modules.php?modname='.$_REQUEST['modname'].'&include_inactive='.$_REQUEST['include_inactive'].'&include_all='.$_REQUEST['include_all'].($_REQUEST['assignment_id']=='all'?'&assignment_id=all':'').(UserStudentID()?'&student_id='.UserStudentID():'').'&type_id=\'+this.options[selectedIndex].value"><OPTION value=""'.(!$_REQUEST['type_id']?' SELECTED':'').'>All</OPTION>';
...
DrawHeader($type_select.$assignment_select,$_REQUEST['assignment_id']?SubmitButton('Save'):'',CheckBoxOnclick('include_inactive')._('Include Inactive Students').' '.CheckBoxOnclick('include_all')._('Include Inactive Assignments'));
if($_REQUEST['assignment_id'] && $_REQUEST['assignment_id']!='all')
{
	...
	switch($column)
	{
		case 'POINTS':
			...
			else
			{
				if($_REQUEST['include_all'] || ($current_RET[$THIS_RET['STUDENT_ID']][$assignment_id][1]['POINTS']!='' || !$assignments_RET[$assignment_id][1]['DUE_EPOCH'] || $assignments_RET[$assignment_id][1]['DUE_EPOCH']>=$THIS_RET['START_EPOCH'] && (!$THIS_RET['END_EPOCH'] || $assignments_RET[$assignment_id][1]['DUE_EPOCH']<=$THIS_RET['END_EPOCH'])))
				{
					...
					return '<TABLE border=0 cellspacing=0 cellpadding=1 class=LO_field><TR><TD>'.TextInput($points,'values['.$THIS_RET['STUDENT_ID'].']['.$assignment_id.'][POINTS]','',' size=2 maxlength=7 tabindex='.$tabindex).'</TD><TD>&nbsp;/&nbsp;</TD><TD>'.$total_points.'</TD></TR></TABLE>';
				}
			}
		break;
		...
		case 'PERCENT_GRADE':
			if(!$assignment_id)
			{
				...
				return ($total>$max_allowed?'<FONT color=red>':'').Percent($total,0).($total>$max_allowed?'</FONT>':'');
			}
			else
			{
				if($_REQUEST['include_all'] || ($current_RET[$THIS_RET['STUDENT_ID']][$assignment_id][1]['POINTS']!='' || !$assignments_RET[$assignment_id][1]['DUE_EPOCH'] || $assignments_RET[$assignment_id][1]['DUE_EPOCH']>=$THIS_RET['START_EPOCH'] && (!$THIS_RET['END_EPOCH'] || $assignments_RET[$assignment_id][1]['DUE_EPOCH']<=$THIS_RET['END_EPOCH'])))
				{
					...
					if($total_points!=0)
						if($points!='-1')
							return ($assignments_RET[$assignment_id][1]['DUE']||$points!=''?($points>$total_points*$max_allowed?'<FONT color=red>':''):'<FONT color=gray>').Percent($points/$total_points,0).($assignments_RET[$assignment_id][1]['DUE']||$points!=''?($points>$total_points*$max_allowed?'</FONT>':''):'');
						else
							return 'N/A';
					else
						return 'E/C';
		...
		case 'LETTER_GRADE':
			...
			else
			{
				if($_REQUEST['include_all'] || ($current_RET[$THIS_RET['STUDENT_ID']][$assignment_id][1]['POINTS']!='' || !$assignments_RET[$assignment_id][1]['DUE_EPOCH'] || $assignments_RET[$assignment_id][1]['DUE_EPOCH']>=$THIS_RET['START_EPOCH'] && (!$THIS_RET['END_EPOCH'] || $assignments_RET[$assignment_id][1]['DUE_EPOCH']<=$THIS_RET['END_EPOCH'])))
				{
					...
					if($total_points!=0)
						if($points!='-1')
							return ($assignments_RET[$assignment_id][1]['DUE']||$points!=''?'':'<FONT color=gray>').'<B>'._makeLetterGrade($points/$total_points).'</B>'.($assignments_RET[$assignment_id][1]['DUE']||$points!=''?'':'</FONT>');
						else
							return 'N/A';
					else
						return 'N/A';
...
{	global $THIS_RET,$assignment_count,$count_assignments,$max_allowed;
	...
	switch($column)
	{
		case 'POINTS':
			...
			return '<TABLE border=0 cellspacing=0 cellpadding=1 class=LO_field><TR><TD>'.TextInput($value,'values['.$THIS_RET['STUDENT_ID'].']['.$THIS_RET['ASSIGNMENT_ID'].'][POINTS]','',' size=2 maxlength=7 tabindex='.$tabindex).'</TD><TD>&nbsp;/&nbsp;</TD><TD>'.$THIS_RET['TOTAL_POINTS'].'</TD></TR></TABLE>';
		break;

		case 'PERCENT_GRADE':
			if($THIS_RET['TOTAL_POINTS']!=0)
				if($THIS_RET['POINTS']!='-1')
					return ($THIS_RET['DUE']||$THIS_RET['POINTS']!=''?($THIS_RET['POINTS']>$THIS_RET['TOTAL_POINTS']*$max_allowed?'<FONT color=red>':''):'<FONT color=gray>').Percent($THIS_RET['POINTS']/$THIS_RET['TOTAL_POINTS'],0).($THIS_RET['DUE']||$THIS_RET['POINTS']!=''?($THIS_RET['POINTS']>$THIS_RET['TOTAL_POINTS']*$max_allowed?'</FONT>':''):'');
				else
					return 'N/A';
			else
				return 'E/C';
		...
		case 'LETTER_GRADE':
			if($THIS_RET['TOTAL_POINTS']!=0)
				if($THIS_RET['POINTS']!='-1')
					return ($THIS_RET['DUE']||$THIS_RET['POINTS']!=''?'':'<FONT color=gray>').'<B>'._makeLetterGrade($THIS_RET['POINTS']/$THIS_RET['TOTAL_POINTS']).'</B>'.($THIS_RET['DUE']||$THIS_RET['POINTS']!=''?'':'</FONT>');
				else
					return 'N/A';
			else
				return 'N/A';
...
{	global $THIS_RET,$assignments_RET,$current_RET,$old_student_id,$student_count,$tabindex,$count_students,$max_allowed;
	...
	if($_REQUEST['include_all'] || ($current_RET[$THIS_RET['STUDENT_ID']][$assignment_id][1]['POINTS']!='' || !$assignments_RET[$assignment_id][1]['DUE_EPOCH'] || $assignments_RET[$assignment_id][1]['DUE_EPOCH']>=$THIS_RET['START_EPOCH'] && (!$THIS_RET['END_EPOCH'] || $assignments_RET[$assignment_id][1]['DUE_EPOCH']<=$THIS_RET['END_EPOCH'])))
	{
		...
		if($total_points!=0)
			if($points!='*')
				return '<TABLE border=0 cellspacing=0 cellpadding=0 class=LO_field><TR align=center><TD>'.TextInput($points,'values['.$THIS_RET['STUDENT_ID'].']['.$assignment_id.'][POINTS]','',' size=2 maxlength=7 tabindex='.$tabindex).'<HR>'.$total_points.'</TD><TD>&nbsp;'.($assignments_RET[$assignment_id][1]['DUE']||$points!=''?($points>$total_points*$max_allowed?'<FONT color=red>':''):'<FONT color=gray>').Percent($points/$total_points,0).($assignments_RET[$assignment_id][1]['DUE']||$points!=''?($points>$total_points*$max_allowed?'</FONT>':''):'').'<BR>&nbsp;<B>'._makeLetterGrade($points/$total_points).'</B>'.'</TD></TR></TABLE>';
			else
				return '<TABLE border=0 cellspacing=0 cellpadding=1 class=LO_field><TR align=center><TD>'.TextInput($points,'values['.$THIS_RET['STUDENT_ID'].']['.$assignment_id.'][POINTS]','',' size=2 maxlength=7 tabindex='.$tabindex).'<HR>'.$total_points.'</TD><TD>&nbsp;'._('N/A').'<BR>&nbsp;'._('N/A').'</TD></TR></TABLE>';
		else
			return '<TABLE border=0 cellspacing=0 cellpadding=1 class=LO_field><TR align=center><TD>'.TextInput($points,'values['.$THIS_RET['STUDENT_ID'].']['.$assignment_id.'][POINTS]','',' size=2 maxlength=7 tabindex='.$tabindex).'<HR>'.$total_points.'</TD><TD>&nbsp;E/C</TD></TR></TABLE>';
	}
}
?>
