...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[mp_tardies] value=Y>'.Localize('colon',_('Other Attendance this quarter')).' <SELECT name="mp_tardies_code">';
		foreach($attendance_codes as $code)
			$extra['extra_header_left'] .= '<OPTION value='.$code['ID'].'>'.$code['SHORT_NAME'].'</OPTION>';
		$extra['extra_header_left'] .= '</SELECT></TD>';
		$extra['extra_header_left'] .= '</TR><TR>';
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		echo '<BR><CENTER><INPUT type=submit value="'._('Create Report Cards for Selected Students').'"></CENTER>';
		echo "</FORM>";
	}
}
