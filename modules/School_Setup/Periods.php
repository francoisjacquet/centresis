...
if($_REQUEST['values'] && $_POST['values'] && AllowEdit())
{
	foreach($_REQUEST['values'] as $id=>$columns)
	{
		...
		else
		{
			...
			$values = db_nextval('SCHOOL_PERIODS').",'".UserSchool()."','".UserSyear()."',";
...
{	global $THIS_RET;
    ...
    if($id!='new' && $value)
        return '<DIV id='.$name.$id.'><div onclick=\'addHTML("<TABLE><TR><TD>'.str_replace('"','\"',SelectInput($hour,'values['.$id.']['.$name.'_HOUR]','',$hour_options,'N/A','',false)).':</TD><TD>'.str_replace('"','\"',SelectInput($minute,'values['.$id.']['.$name.'_MINUTE]','',$minute_options,'N/A','',false)).'</TD><TD>'.str_replace('"','\"',SelectInput($m,'values['.$id.']['.$name.'_M]','',$m_options,'N/A','',false)).'</TD></TR></TABLE>","'.$name.$id.'",true);\'>'."<span style='border-bottom-style:dotted;border-bottom-width:1px;border-bottom-color:".Preferences('TITLES').";'>".$value.'</span></div></DIV>';
    else
