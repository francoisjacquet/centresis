...
{
    $RET = DBGet(DBQuery("SELECT fst.TRANSACTION_ID AS TRANS_ID,fst.TRANSACTION_ID,fst.ACCOUNT_ID,fst.SHORT_NAME,fst.STUDENT_ID,fst.DISCOUNT,(SELECT sum(AMOUNT) FROM FOOD_SERVICE_TRANSACTION_ITEMS WHERE TRANSACTION_ID=fst.TRANSACTION_ID) AS AMOUNT,fst.BALANCE,DATE_FORMAT(fst.TIMESTAMP,'%Y-%m-%d') AS DATE,DATE_FORMAT(fst.TIMESTAMP,'%h:%i:%S %p') AS TIME,fst.DESCRIPTION,".db_case(array('fst.STUDENT_ID',"''",'NULL',"(SELECT CONCAT(LAST_NAME,', ',".(Preferences('NAME')=='Common'?'coalesce(CUSTOM_200000002,FIRST_NAME)':'FIRST_NAME').") FROM STUDENTS WHERE STUDENT_ID=fst.STUDENT_ID)"))." AS FULL_NAME,".db_case(array('fst.SELLER_ID',"''",'NULL',"(SELECT CONCAT(FIRST_NAME,' ',LAST_NAME) FROM staff WHERE STAFF_ID=fst.SELLER_ID)"))." AS SELLER FROM FOOD_SERVICE_TRANSACTIONS fst WHERE SYEAR='".UserSyear()."' AND fst.TIMESTAMP BETWEEN '".date("Y-m-d",strtotime($date))."' AND date '".date("Y-m-d",strtotime('+1 day', strtotime($date)))."' AND SCHOOL_ID='".UserSchool()."'".$where."ORDER BY ".($_REQUEST['by_name']?"FULL_NAME,":'')."fst.TRANSACTION_ID DESC"),array('DATE'=>'ProperDate','SHORT_NAME'=>'bump_count'));
