...
if($_REQUEST[modfunc]=='list' && ($_REQUEST[editor]=='stubilling' || $_REQUEST[editor]=='lunch'))
{
	...
	$sql = "select school,ADDRESS1,zipcode,telephone,city,state from schools";
	$address_RET = DBGet(DBQuery($sql),array(),array('SCHOOL'));
	foreach($address_RET as $school=>$address)
	{
		$address = $address[1];
		$school_address[$school] = '<font size=+1>'.GetTitle() . '</font><BR>' . $address['ADDRESS1'] . '<BR>' . $address['CITY'].', '.$address['STATE'].' '.$address['ZIPCODE'].'<BR>'$address['TELEPHONE'];
	}
